...
Umbra uses BT limit entry orders at the fresh last-traded price, without a stop-loss or automatic exit. Settings are rupees per stock and entry time in IST. Positive influence maps to SELL; Negative influence maps to BUY. The previously supported exit-time setting is ignored, including in older saved settings and run receipts.
