...
class LimitTests(unittest.TestCase):
    ...
    def test_both_sides_send_explicit_positive_price(self):
        for side in ("BUY", "SELL"):
            self.broker.place("AAA", 123, side, 9, Decimal("101.05"), stop_price="100" if side == "BUY" else "102", target_price="103" if side == "BUY" else "99")
            payload = self.broker.client.placeOrder.call_args.args[0]
            self.assertEqual(payload["price"], "101.05")
            self.assertEqual(payload["orderType"], "BKT")
    ...
    def test_invalid_price_never_reaches_broker(self):
        for price in (None, "", "bad", "0", "-1", "NaN", "Infinity"):
            with self.subTest(price=price), self.assertRaises(ValueError):
                self.broker.place("AAA", 123, "BUY", 1, price, stop_price="100", target_price="103")
    ...
    def test_rejection_has_no_market_fallback(self):
        ...
        with self.assertRaises(ValueError):
            self.broker.place("AAA", 123, "BUY", 1, "101.05", stop_price="100", target_price="103")
